...
async function createDirectory(directoryUrl) {
  try {
    ...
    if (response.statusCode === 201) {
      ...
      console.log(`Directory conflict (409) - may need to create parent directories first: ${directoryUrl}`);
      // 尝试创建父目录
      ...
      console.error(`Error creating directory ${directoryUrl}: ${response.statusCode} - ${response.data}`);
      return false;
    }
  } catch (error) {
